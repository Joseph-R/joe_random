...
* Milty Draft - Game parameters, including Milty draft settings, are always available for discussion amongst the players confirmed for game day.  Our primary use for Milty right now is to speed up setup on game days.  We default to the following:
    ...
    * Factions - Player count + 6.  Faction choices may be seeded (by table agreement) based on weird/funky game modes.
    * Neighboring Red Tiles - Once the draft is complete and the map is resolved, starting with last position and going in ascending draft order, any player whose slice is impacted by two red tiles adjacent to each other will have the choice of swapping one of the red tiles in their slice with an adjacent blue tile within their slice.  They cannot move a red tile from a red tile adjacent with one player to a red tile adjacent with another.  The player may also pass. If their action/inaction creates a "two red tile adjacent" issue for the next player up the order draft order, that player gets the same choice, cascading until the map creation rules are satisfied or all impacted players have passed.
        * Why? - We had three red tiles adjacent once, blocking one player from any realistic path to Mecatol in a 13+ hour game.  We never saw that player again.
* Please talk through your turn - "I'm doing XYZ which triggers PQN" is fine.  Action logs help players learn, keep everyone honest, and can help players realize/avoid honest mistakes (like forgetting a Sarween/AI Dev discount, a commander alliance ability, etc).
* [Support for the Throne](https://twilight-imperium.fandom.com/wiki/Promissory_Notes#Support_for_the_Throne) - There are reams written about this online.  We don't have any rules.  We strongly encourage players to make their Support deals by the end of Round 4.  But if you want to incur a blood vendetta by giving someone a Support point for the win, feel free!  Just don't be surprised if one or more players are gunning for you next game. 
